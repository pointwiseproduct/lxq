...
public:
    ...
    public:
        ...
        term_sequence(std::initializer_list<term> list) : std::vector<term>(list){}
    ...
    struct lr_parsing_table_item_comparetor{
        bool operator ()(lr_parsing_table_item const &lhs, lr_parsing_table_item const &rhs) const{
            ...
            return item_comparetor()(*lhs.item_ptr, *rhs.item_ptr);
        }
    };
    ...
    void lr1_items(grammar const &g, states &c, typename states::iterator &first_state, typename items::iterator &first_item, term_set const &terminal_symbol_set, item const &start){
        {
            items init;
            init.insert(start);
    ...
    make_result make2(
        ...
        auto insert_conflict = [&](lr_parsing_table_item const &x, lr_parsing_table_item const &y){
            if(lr_parsing_table_item_comparetor()(x, y)){
                conflict_set.insert(lr_conflict({ x, y }));
            }else{
                conflict_set.insert(lr_conflict({ y, x }));
            }
        };
            ...
            for(item const &i : state){
                if(i.is_over()){
                    if(i.lhs == start.lhs && i.rhs == start.rhs && i.lookahead == start.lookahead){
                        ...
                        if(!p.second && (p.first->second.num != a.num || !p.first->second.item_ptr->equal(*a.item_ptr))){
                        ...
                        for(term const &t : i.lookahead){
                            ...
                            if(!p.second){
                                ...
                                if(other.action == lr_parsing_table_item::enum_action::reduce){
                                    if(other.num != a.num || !other.item_ptr->equal(*a.item_ptr)){
                                    ...
                                    if(symbol_data.priority > other_symbol_data.priority){
                                        ...
                                        if(symbol_data.dir == linkdir::left){
                                            if(other_symbol_data.dir != linkdir::left){
                                                p.first->second = a;
                                            }
                                        }
                                    }
                            ...
                            if(other.action == lr_parsing_table_item::enum_action::reduce){
                                ...
                                if(symbol_data.priority == other_symbol_data.priority){
                                    if(symbol_data.dir == linkdir::right){
                                        if(other_symbol_data.dir != linkdir::right){
                                            p.first->second = a;
                                        }
                                    }
                                }
